...
public class AdManager : Singleton<AdManager>
{
    ...
    string appKey;
    /// <summary>
    /// The game ID which is associated with this app. It is necessary to make sure that 'Unity Ads' work properly. It can be looked up on the 
    /// Unity Dashboard, in the details section of this Unity project.
    /// </summary>
    string gameId;
    /// <summary>
    ...
    bool vibrationOn;
    /// <summary>
    /// If true, Appodeal Ads should be played, otherwise Unity Ads
    /// </summary>
    bool AppodealAds
    {
        get { return StaticValues.appodealAds; }
    }
    ...
    void Start()
    {
        ...
        gameId = "0e8ca825-be87-4624-b617-a0521b99102a";
        consent = FullVersion.Instance.CollectionOfDataConsent == AdDataCollectionPermitted.permitted ? true : false;
        Appodeal.disableLocationPermissionCheck();
        Appodeal.setTesting(StaticValues.AdsTestingEnabled);

        //if (AppodealAds)
            Appodeal.initialize(appKey, Appodeal.INTERSTITIAL | Appodeal.NON_SKIPPABLE_VIDEO | Appodeal.BANNER | Appodeal.REWARDED_VIDEO, consent);
        //else
         //   Advertisement.Initialize(gameId, StaticValues.AdsTestingEnabled);
    ...
    public void ShowVideoAdOnLose(bool gameWon)
    {
        ...
        { 
            ...
            else if (adCounter >= 120)
            {
                data = ShowInterstitialOrVideo(1, data);
            }
            else if (adCounter >= 180)
            {
    ...
    public void ShowVideoAdOnLose(bool gameWon, SoundMethod playGameOverSound, SoundMethod playVibration)
    {
        ...
        if (gameWon)
        { 
            playGameOverSound(soundOn, gameWon);
            playVibration(vibrationOn, gameWon);
        }
        else
        {
            FullVersionData data = FullVersion.Instance.RetrieveFullVersionDataFromFile();
    ...
    /// <summary>
    /// Plays an ad that is rewarded.
    /// </summary>
    /// <param name="adCantBeLoaded">The method which should be executed if the ad wasn't loaded.</param>
    /// <param name="bugReport">The text which is shown if the rewarded ad couldn't be played.</param>
    public void ShowRewardedAd(MethodWithOneParameter adCantBeLoaded, string bugReport)
    {
        if (FullVersion.Instance.CollectionOfDataConsent != AdDataCollectionPermitted.notSet)
        {
            if (Appodeal.isLoaded(Appodeal.REWARDED_VIDEO))
                ...
                adCantBeLoaded(bugReport);
        }
        else
        {
            userConsent = PlayRewardedVideo;
    ...
    /// <summary>
    /// A rewarded video is played without priorly testing whether the ads were even initialized.
    /// </summary>
    ...
    public FullVersionData ShowInterstitial(FullVersionData thisData, SoundMethod playGameOverSound, SoundMethod playVibration)
    {
        ...
        {
            playGameOverSound(soundOn, false);
            playVibration(vibrationOn, false);
        }
        return thisData;
    }
    ...
    // 
    //
    //          METHODS FOR IMPLEMENTING UNITY ADS - FROM NOW ON:
    //
    //
